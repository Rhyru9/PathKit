...
| **Weak-labeled candidates** | 16,581 | subset of train pool | heuristic |
| **Validation** | 3,316 | deterministic 20% of weak candidates, seed=2027 | weak labels |
| **Development-held-out test** | 500 | `sample_labeled.csv` | heuristic-assisted review (1 annotator) |
...
- [x] A dedicated weak-label validation split -> `scripts/create_validation_split.py`;
      metadata is tracked in `data/splits.json`, while raw validation paths remain
      ignored.
